...
namespace {
struct ReturnDepVisitor {
    TrainStation &station;
    TrainStation outboundArrival;
...
struct ReturnArrVisitor {
    TrainStation &station;
    TrainStation outboundDeparture;
    template<typename T> void operator()(T &&nrt) const {
        if (nrt.returnIncluded && nrt.returnDescriptionIsSet()) {
...
QString Uic9183Parser::pnr() const
{
    ...
    {
        const auto flex = findBlock<Uic9183Flex>();
        if (flex.isValid()) {
        {
            auto pnr = FcbExtractor::pnr(flex.fcb());
            if (!pnr.isEmpty()) {
            return pnr;
        }
    }
...
QDateTime Uic9183Parser::validFrom() const
{
    ...
    // CD vender block
    {
        const auto b = findBlock<Vendor1154UTBlock>();
        if (b.isValid()) {
...
TrainStation Uic9183Parser::outboundDepartureStation() const
{
    ...
    // DB vendor block
    {
        ...
        if (b.isValid()) {
        {
            const auto sblock = b.findSubBlock("015");
            if (!sblock.isNull()) {
...
TrainStation Uic9183Parser::outboundArrivalStation() const
{
...

    {
        ...
        if (b.isValid()) {
        {
            const auto sblock = b.findSubBlock("016");
            if (!sblock.isNull()) {
            station.setName(sblock.toString());
        }
    }
...
TrainStation Uic9183Parser::returnDepartureStation() const
{
    ...
    // DB vendor block
    {
        ...
        if (b.isValid()) {
        {
            const auto sblock = b.findSubBlock("017");
            if (!sblock.isNull()) {
            station.setName(sblock.toString());
        }
    }
    ...
    // ERA FCB
    {
        ...
        makeVariantVisitor(ReturnDepVisitor{station, outboundArrival}).template visit<FCB_VERSIONED(OpenTicketData)>(doc);
...
TrainStation Uic9183Parser::returnArrivalStation() const
{
    ...
    // DB vendor block
    {
        ...
        if (b.isValid()) {
        {
            const auto sblock = b.findSubBlock("018");
            if (!sblock.isNull()) {
            station.setName(sblock.toString());
        }
    }
    ...
    // ERA FCB
    {
        ...
        if (flex.hasTransportDocument()) {
        const auto doc = flex.transportDocuments().at(0);
        makeVariantVisitor(ReturnArrVisitor{station, outboundDeparture}).template visit<FCB_VERSIONED(OpenTicketData)>(doc);
        FcbExtractor::fixStationCode(station);
    }
    }
...
{
    {
        ...
        if (flex.isValid()) {
        {
            auto c = FcbExtractor::seatingType(flex.fcb());
            if (!c.isEmpty()) {
            return c;
        }
    }
