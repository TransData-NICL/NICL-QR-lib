...
template <typename T>
static typename std::enable_if<!std::is_same<T, QString>::value, bool>::type equalAndPresent(const T &lhs, const T &rhs)
{
...
template <typename T>
static typename std::enable_if<!std::is_same<T, QString>::value, bool>::type conflictIfPresent(const T &lhs, const T &rhs)
{
...
static bool conflictIfPresent(const QDate &lhs, const QDate &rhs)
{
    return lhs.isValid() && rhs.isValid() && lhs != rhs;
}
...
bool isSameSeat(const Seat &lhsSeat, const Seat &rhsSeat)
{
    ...
    if (conflictIfPresent(lhsSeat.seatNumber(), rhsSeat.seatNumber(), Qt::CaseInsensitive)) {
        ...
        std::sort(lhsSet.begin(), lhsSet.end());
        std::sort(rhsSet.begin(), rhsSet.end());
...
{
    QMetaType lhsMt(lhs.userType());
    QMetaType rhsMt(rhs.userType());
...
#else
    const QMetaObject *current = lhsMo;
    while (current) {
        if (current == rhsMo)
            return true;
...
static bool isSameLineName(const QString &lhs, const QString &rhs)
{
    if (isSameLineName(lhs.begin(), lhs.end(), rhs.begin(), rhs.end())
        || isSameLineName(std::reverse_iterator<QString::const_iterator>(lhs.end()), std::reverse_iterator<QString::const_iterator>(lhs.begin()), std::reverse_iterator<QString::const_iterator>(rhs.end()), std::reverse_iterator<QString::const_iterator>(rhs.begin())))
    {
        return true;
    }
    ...
    return isSameTrainName(lhsMatch.captured(QStringLiteral("type")), rhsMatch.captured(QStringLiteral("type"))) && (
           (equalAndPresent(lhsMatch.captured(QStringLiteral("line")), rhsMatch.captured(QStringLiteral("line"))) && !conflictIfPresent(lhsMatch.captured(QStringLiteral("route")), rhsMatch.captured(QStringLiteral("route"))))
        || (equalAndPresent(lhsMatch.captured(QStringLiteral("line")), rhsMatch.captured(QStringLiteral("route"))) && lhsMatch.captured(QStringLiteral("route")).isEmpty())
        || (equalAndPresent(lhsMatch.captured(QStringLiteral("route")), rhsMatch.captured(QStringLiteral("line"))) && rhsMatch.captured(QStringLiteral("route")).isEmpty())
    );
}
...
bool MergeUtil::isSamePerson(const Person& lhs, const Person& rhs)
{
    ...
    if (lhs.givenName().isEmpty() && lhs.familyName().isEmpty() && !lhs.name().isEmpty()
     && rhs.givenName().isEmpty() && rhs.familyName().isEmpty() && !rhs.name().isEmpty()) {
        ...
        std::sort(lhsSplit.begin(), lhsSplit.end());
        std::sort(rhsSplit.begin(), rhsSplit.end());
...
QVariant MergeUtil::merge(const QVariant &lhs, const QVariant &rhs)
{
    ...
    if (lhs.userType() != rhs.userType() && !isSubType(lhs, rhs) && !isSubType(rhs, lhs)) {
    ...
    if (lhs.userType() == rhs.userType()) {
        ...
        {
            QMetaType _mt(mt);
            if ((_mt.flags() & QMetaType::IsGadget) == 0 && !JsonLd::valueIsNull(rhs)) {
                return rhs;
            }
        }
    }
    ...
    QMetaType mt2(res.userType());
    const auto mo = mt2.metaObject();
    if (mo && (mt2.flags() & QMetaType::IsGadget)) {
        for (int i = 0; i < mo->propertyCount(); ++i) {
            ...
            if (rv.userType() == QMetaType::QVariantList && std::strcmp(prop.name(), "subjectOf") == 0) {
