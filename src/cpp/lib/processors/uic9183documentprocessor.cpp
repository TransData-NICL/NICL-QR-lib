...
namespace {
struct Uic9183CurrencyVisitor { template<typename T> QString operator()(T &&fcb) const { return QString::fromUtf8(fcb.issuingDetail.currency); } };
}
...
void Uic9183DocumentProcessor::expandNode(ExtractorDocumentNode &node, const ExtractorEngine * /*engine*/) const
{
    ...
    if (!node.contextDateTime().isValid()) {
        ...
        const auto u_flex = p.findBlock<Uic9183Flex>();
        if (u_flex.isValid()) {
            node.setContextDateTime(FcbExtractor::issuingDateTime(u_flex.fcb()));
        } else {
...
void Uic9183DocumentProcessor::preExtract(ExtractorDocumentNode &node, const ExtractorEngine * /*engine*/) const
{
    const auto p = node.content<Uic9183Parser>();
    ...
    Seat seat;
    {
        const auto seatingType = p.seatingType();
    ...
    if (rct2.isValid()) {
        ...
        {
            const auto currency = rct2.currency();
            if (!currency.isEmpty()) {
    ...
    {
        const auto flex = p.findBlock<Uic9183Flex>();
        if (flex.isValid()) {
            // get currency from FCB issuing detail
#if __cplusplus >= 201703L
            res.setPriceCurrency(std::visit([](auto &&fcb) { return QString::fromUtf8(fcb.issuingDetail.currency); }, flex.fcb()));
#else
            res.setPriceCurrency(
                makeVariantVisitor(Uic9183CurrencyVisitor{})
                    .template visit<FCB_VERSIONED(UicRailTicketData)>(flex.fcb().toVariant())
            );
#endif
