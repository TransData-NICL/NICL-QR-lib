...
[[nodiscard]] static TrainStation makeStation(int idType, const QString &alphaId, int numericId)
{
    ...
    if (idType == 0 && numericId > 1000000 && numericId < 9999999) {
        station.setIdentifier("uic:"_L1 + QString::number(numericId));
        station.setName(QString::number(numericId));
    } else if (idType == 0 && numericId > 10000000 && numericId < 99999999 && UicStationCode::checksumDigit(numericId / 10) == numericId % 10) {
    ...
    if (node.isA<SSBv3Ticket>()) {
        ...
        switch (ssb.ticketTypeCode()) {
            ...
            case SSBv3Ticket::NRT:
                ...
                ticket.setValidFrom(QDateTime(ssb.type2ValidFrom(node.contextDateTime()), QTime(0, 0)));
                ticket.setValidUntil({ssb.type2ValidUntil(node.contextDateTime()), {23, 59, 59}});
                {
                    const auto t = ssb.type2OpenText().trimmed();
                    if (!t.isEmpty()) {
                        ticket.setName(t);
                    }
                }
            ...
            case SSBv3Ticket::GRT:
            {
                ...
                ticket.setValidFrom(QDateTime(ssb.type3ValidFrom(node.contextDateTime()), QTime(0, 0)));
        ...
        PriceFinder finder;
        {
            const auto price = finder.findHighest(ticket.name());
            if (price.hasResult()) {
