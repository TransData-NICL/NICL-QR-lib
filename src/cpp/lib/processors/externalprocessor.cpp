...
ExternalProcessor::ExternalProcessor()
{
	...
	const QString filepath =
		...
		QLatin1StringView( CMAKE_INSTALL_FULL_LIBEXECDIR_KF5 ) +
#else
		QLatin1StringView( CMAKE_INSTALL_FULL_LIBEXECDIR_KF5 ) +
#endif
...
void ExternalProcessor::preExtract(ExtractorDocumentNode &node, const ExtractorEngine *engine) const
{
    ...
    std::transform(extractors.begin(), extractors.end(), std::back_inserter(extNames), [](const AbstractExtractor *ext) { return ext->name(); });
