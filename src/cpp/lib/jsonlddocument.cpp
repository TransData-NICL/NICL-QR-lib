...
static void registerBuiltInTypes(std::vector<TypeInfo> &r)
{
    ...
    assert(std::is_sorted(r.begin(), r.end(), [](const TypeInfo &lhs, const TypeInfo &rhs) {
...
static QVariant createInstance(const QJsonObject& obj, const QString &type)
{
    ...
    const auto it = std::lower_bound(registry.begin(), registry.end(), type,
                                     [](const TypeInfo &lhs, const QString &rhs) {
                                       return QLatin1StringView(lhs.name) < rhs;
                                     });
    if (it != registry.end() && QLatin1StringView((*it).name) == type) {
                        QVariant value;
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
                        value = QVariant((*it).metaTypeId, static_cast<const void*>(nullptr));
#else
                        value = QVariant(QMetaType((*it).metaTypeId), static_cast<const void*>(nullptr));
#endif
...
static QVariant createInstance(const QJsonObject &obj, const QMetaProperty &prop)
{
    ...
    if (mo) {
        QVariant value;
    #if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
        value = QVariant(prop.userType(), static_cast<const void*>(nullptr));
    #else
        value = QVariant(QMetaType(prop.userType()), static_cast<const void*>(nullptr));
    #endif
...
#else
            QVariant value(mt, static_cast<const void*>(valueData));
#endif
            mt.destroy(valueData);
...
void JsonLdDocument::registerType(const char *typeName, const QMetaObject *mo, int metaTypeId)
{
    ...
    const auto it = std::lower_bound(registry.begin(), registry.end(), typeName, [](const TypeInfo &lhs, const char *rhs) {
