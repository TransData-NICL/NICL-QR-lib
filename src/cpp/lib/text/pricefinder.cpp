...
PriceFinder::PriceFinder()
{
    ...
    // remove duplicates
    const auto lessThanCurrencyData = [](const CurrencyData &lhs, const CurrencyData &rhs) {
    ...
    const auto compareCurrencyData = [](const CurrencyData &lhs, const CurrencyData &rhs) {
    ...
    for (auto it = s_currencyData.begin(); it != s_currencyData.end(); ++it) {
        const auto it2 = std::lower_bound(std::begin(currency_data_overrides), std::end(currency_data_overrides), (*it).isoCode, [](decltype(currency_data_overrides[0]) &lhs, const QString &rhs) {
...
void PriceFinder::findAll(QStringView text, std::vector<Result> &results) const
{
    ...
    qsizetype offset = 0;
    while (true) {
        const auto match = rx.match(text.toString(), offset);
        if (!match.hasMatch()) {
            break;
        }
        offset = match.capturedEnd(2);

        const auto leadingCurrency = parseCurrency(match.captured(1), CurrencyPrefix);
        const auto trailingCurrency = parseCurrency(match.captured(3), CurrencySuffix);
        ...
        r.value = parseValue(match.captured(2), r.currency);
...
bool PriceFinder::isSingleCurrency(const std::vector<Result> &results) const
{
    ...
    return std::all_of(results.begin(), results.end(), [&isoCode](const Result &r) { return r.currency == isoCode; });
}
...
PriceFinder::Result PriceFinder::highest(std::vector<Result> &results) const
{
    ...
    std::sort(results.begin(), results.end(), [](const Result &lhs, const Result &rhs) { return lhs.value > rhs.value; });
    if (results.size() == 1) {
    ...
    const auto it = std::find_if(std::begin(price_upper_limit), std::end(price_upper_limit), [&results](decltype(price_upper_limit[0]) &t) {
...
QString PriceFinder::parseCurrency(QStringView s, CurrencyPosition pos) const
{
    ...
    if (isoCandidate.size() == 3) {
        const auto it = std::lower_bound(s_currencyData.begin(), s_currencyData.end(), isoCandidate, [](const CurrencyData &lhs, QStringView rhs) { return lhs.isoCode < rhs; });
    ...
    const auto it = std::find_if(s_currencyData.begin(), s_currencyData.end(), [&symbol](const CurrencyData &data) { return data.symbol == symbol; });
