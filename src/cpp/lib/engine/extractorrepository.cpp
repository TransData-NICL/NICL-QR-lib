...
#include "logging.h"
#include "extractors/activitypubextractor.h"
...
{
    addExtractor(std::make_unique<ActivityPubExtractor>());
...
void ExtractorRepository::extractorsForNode(const ExtractorDocumentNode &node, std::vector<const AbstractExtractor*> &extractors) const
{
    ...
    for (const auto &extractor : d->m_extractors) {
        if (extractor->canHandle(node)) {
            ...
            const auto it = std::lower_bound(extractors.begin(), extractors.end(), extractor.get(), [](const AbstractExtractor *lhs, const AbstractExtractor *rhs) {
                return lhs < rhs;
            });
...
{
    auto it = std::lower_bound(d->m_extractors.begin(), d->m_extractors.end(), name, [](const std::unique_ptr<AbstractExtractor> &lhs, QStringView rhs) {
        return lhs->name() < rhs;
    });
...
{
    auto it = std::lower_bound(m_extractors.begin(), m_extractors.end(), e, [](const std::unique_ptr<AbstractExtractor> &lhs, const std::unique_ptr<AbstractExtractor> &rhs) {
