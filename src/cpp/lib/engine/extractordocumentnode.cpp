...
QVariantList ExtractorDocumentNode::childNodesVariant() const
{
    ...
    std::transform(d->childNodes.begin(), d->childNodes.end(), std::back_inserter(l), [](const ExtractorDocumentNode &c) { return QVariant::fromValue(c); });
    return l;
}
...
QVariantList ExtractorDocumentNode::findChildNodes(const QJSValue &jsFilter) const
{
    ...
    std::transform(matches.begin(), matches.end(), std::back_inserter(l), [](const ExtractorDocumentNode &c) { return QVariant::fromValue(c); });
    return l;
}
