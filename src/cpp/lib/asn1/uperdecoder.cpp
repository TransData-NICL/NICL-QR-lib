...
int64_t UPERDecoder::readConstrainedWholeNumber(int64_t minimum, int64_t maximum)
{
    ...
    // Count leading zeros for 64-bit value (compatible with Qt < 5.6 which lacks qCountLeadingZeroBits)
    size_type bits = 0;
    if (range > 0) {
        uint64_t tmp = range;
...
QByteArray UPERDecoder::readObjectIdentifier()
{
    ...
    QByteArray oid = QByteArray::number(static_cast<qulonglong>(value / 40)) + '.' + QByteArray::number(static_cast<qulonglong>(value % 40));
