...
QString LocationUtil::formatAddress(const PostalAddress &addr)
{
#if HAVE_KCONTACTS
    return toAddress(addr).formatted(KContacts::AddressFormatStyle::MultiLineInternational);
#else
    QStringList parts;
    if (!addr.streetAddress().isEmpty()) parts.push_back(addr.streetAddress());
    if (!addr.postalCode().isEmpty() || !addr.addressLocality().isEmpty())
        parts.push_back((addr.postalCode() + QLatin1Char(' ') + addr.addressLocality()).trimmed());
    if (!addr.addressRegion().isEmpty()) parts.push_back(addr.addressRegion());
    if (!addr.addressCountry().isEmpty()) parts.push_back(addr.addressCountry());
...
GeoCoordinates LocationUtil::geoFromUrl(const QUrl &url)
{
    if (url.host().contains(QLatin1StringView("google"))) {
        ...
        if (match.hasMatch()) {
            const auto latitude = match.captured(1).toDouble();
            const auto longitude = match.captured(2).toDouble();
...
#else
        query.addQueryItem(QStringLiteral("q"), formatAddress(addr).replace(QLatin1Char('\n'), QLatin1String(", ")));
#endif
        url.setQuery(query);
        return url;
    }
