...
{
    const auto letterCount = std::count_if(s.begin(), s.end(), [](QChar c) { return c.isLetter(); });
    const auto upperCount = std::count_if(s.begin(), s.end(), [](QChar c) { return c.isUpper(); });
...
QString StringUtil::transliterate(QStringView s)
{
    ...
    for (const auto c : s) {
        const auto it = std::lower_bound(std::begin(transliteration_map), std::end(transliteration_map), c, [](decltype(transliteration_map[0]) &lhs, QChar rhs) {
