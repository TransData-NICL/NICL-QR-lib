...
static void filterPlace(QJsonObject &obj)
{
    ...
    {
        const auto addr = obj.value("address"_L1);
        if (addr.isString()) {
...
static void filterEvent(QJsonObject &obj)
{
    ...
    {
        const auto endDate = obj.value("endDate"_L1).toString();
        if (endDate.size() == 10) {
            const auto date = QDate::fromString(endDate, Qt::ISODate);
            if (date.isValid()) {
                obj.insert("endDate"_L1, QDateTime(date, QTime(23, 59, 59)).toString(Qt::ISODate));
            }
        }
    }
}
