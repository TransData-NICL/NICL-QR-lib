...
QDateTime ELBTicket::validFromDate(const QDateTime &contextDate) const
{
    ...
    return QDateTime(d.addDays(beginValidityDay() - 1), QTime(0, 0));
}
...
QDateTime ELBTicket::validUntilDate(const QDateTime &contextDate) const
{
    ...
    return QDateTime(d.addDays(endValidityDay() -1), QTime(0, 0));
}
...
bool ELBTicketSegment::isValid() const
{
    ...
    return departureDay() > 0 && (classOfTransport() == QString(QLatin1Char('1')) || classOfTransport() == QString(QLatin1Char('2')));
}
