...
static QString ticketName(const Fcb::UicRailTicketData &fcb);

    /** Reference number/PNR. */
static QString pnr(const Fcb::UicRailTicketData &fcb);

    /** Seating class, if available. */
static QString seatingType(const Fcb::UicRailTicketData &fcb);

    /** Issuer organization of this ticket. */
static QString issuerId(const Fcb::UicRailTicketData &fcb);
static Organization issuer(const Fcb::UicRailTicketData &fcb);
    /** Traveller who this ticket is for. */
static Person person(const Fcb::UicRailTicketData &fcb);

    /** Issuing date/time. */
static QDateTime issuingDateTime(const Fcb::UicRailTicketData &fcb);
    /** Ticket validity range. */
static QDateTime validFrom(const Fcb::UicRailTicketData &fcb);
static QDateTime validUntil(const Fcb::UicRailTicketData &fcb);
...
private:
    static PriceData price(const Fcb::UicRailTicketData &fcb);
};
