...
#define FCB_READ_ENUM(Name) \
    do { if (Name ## IsSet()) { \
        Name = uperHasExtensionMarker(decltype(Name){}) \
            ? decoder.readEnumeratedWithExtensionMarker<decltype(Name)>(uperEnumKeyCount(decltype(Name){})) \
            : decoder.readEnumerated<decltype(Name)>(uperEnumKeyCount(decltype(Name){})); \
    } } while(false)
